...
class CustomUserManager(BaseUserManager):
    def create_user(self, email, password=None, cpf=None,**extra_fields):
        if None in (email,password,cpf): #Se não tiver email, senha ou cpf
            raise ValueError("Campo email, senha ou cpf não informados!") #Se faltar algum campo vai gerar esse erro. Raise é para jogar o erro
    
        email_ok = self.normalize_email(email)

        extra_fields.setdefault("is_active", True)

        #prepara para salvar no banco (constroi o objeto)
